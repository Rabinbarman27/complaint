...
$sql = "INSERT INTO feedback_complaint_data 
        ...
        ('$Operation','$Given_by','$Date_of_submission','$Depatment_Section',
         ...
         '$active_error','$latent_error','$cognitive_error','$non_cognitive_error',
         '$root_cause',$avg_impact_score_sql,$avg_freq_score_sql,
         '$immediate_correction','$corrective_action','$preventive_action','$patient_consequences',
...
if ($result) {
    ...
    // TEMP DEBUG: shows the real Postgres error in the response.
    // Once everything works, switch back to the generic message below
    // and keep only the error_log() call for production.
    $debug_error = pg_last_error($conn);
    error_log("SQL Error: " . $debug_error);
    echo json_encode(['success' => false, 'error' => $debug_error]);
